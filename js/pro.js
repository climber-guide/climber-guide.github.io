...
class ProChart { // eslint-disable-line no-unused-vars
  ...
  /**
   * @param {Pro.RANGE|Pro.WEIGHT} scale
   * @param {number} percent - % to pad data (ex: 0.20 -> 20%)
   */
  ...
  #refreshChart () {
    ...
    this.chart.data.datasets = [
      {
        ...
        // backgroundColor: ProChart.COLOR.TEXT.DARK,
        // borderColor: ProChart.COLOR.WEIGHT + '90',
        pointBackgroundColor: ProChart.COLOR.TEXT.DARK,
        ...
        datalabels:
        {
          labels:
          {
            brandModelSize:
            {
              ...
              anchor: 'center',
              offset: 16,
              color: ProChart.COLOR.TEXT.DARK,
              font:
              {
                weight: 'bold'
              },
        ...
        datalabels:
        {
          labels:
          {
            strengthAndWeight:
            {
              ...
              formatter: (value, context) => {
                const pro = this.#proList[context.dataIndex]
                if ([Pro.TYPE.HEX, Pro.TYPE.NUT].includes(pro.type) === false) return ''
                let data = pro.weight + ' g │ '
                if (pro.strength.p1 === pro.strength.p2) data += pro.strength.p1+ ' kN'
                else pro.strength.p1 + '/' + pro.strength.p2+ ' kN'
                return data
              }
            }
          }
      ...
      {
        // data
        label: Pro.RANGE.OPERATION + 'al Range (mm)',
        ...
        borderColor: ProChart.COLOR.TEXT.DARK,
        // size
        barPercentage: 0.85,
      ...
      },
      {
        ...
        backgroundColor: (context) => {
          if (this.#proList.length === 0) return ProChart.COLOR.TEXT.LIGHT
          if (context.chart.legend.legendItems.length === 0) return ProChart.COLOR.TEXT.LIGHT
          if (context.chart.legend.legendItems[0].hidden === false) return ProChart.COLOR.TEXT.LIGHT
          if (this.#proList.length) return this.#proList[context.dataIndex].color
          else return ProChart.COLOR.MAGENTA
        },
        borderColor: ProChart.COLOR.TEXT.DARK,
        // size
        barPercentage: 0.85,
        ...
        datalabels:
        {
          labels:
          {
            ...
            expansionDelta:
            {
              ...
              anchor: 'center',
              color: (context) => {
                if (this.#proList.length === 0) return 'none'
                const pro = this.#proList[context.dataIndex]
                if (pro.color === Pro.COLOR.BLACK) return ProChart.COLOR.TEXT.LIGHT
                else return ProChart.COLOR.TEXT.DARK
              },
              ...
              formatter: (value, context) => {
                const pro = this.#proList[context.dataIndex]
                const len = context.chart.legend.legendItems.length
                if ([Pro.TYPE.HEX, Pro.TYPE.NUT].includes(pro.type)) return ''
                if (context.chart.legend.legendItems[len - 1].hidden === false) return ''
                return (value[1] - value[0]).toFixed(1)
              }
            },
            strengthAndWeight:
            {
              ...
              formatter: (value, context) => {
                ...
                if ([Pro.TYPE.HEX, Pro.TYPE.NUT].includes(pro.type)) return ''
                let data = pro.weight + ' g │ '
                if (pro.strength.active === pro.strength.p1) data += pro.strength.active + ' kN'
                else {
                  const p1 = pro.strength.p1 ? pro.strength.p1 : 'x'
                  data += pro.strength.active + '/' + p1 + ' kN'
                }
                return data
              }
            }
    ...
    this.chart.options.scales.x = {
      alignToPixels: true,
      position: 'top',
      min: this.#chartScale(Pro.RANGE, 0.12).min,
      max: this.#chartScale(Pro.RANGE, 0.12).max,
      grid:
      {
        color: ProChart.COLOR.RANGE + '90'
      },
      ticks:
      {
        color: ProChart.COLOR.RANGE,
        font:
        {
          weight: 'bold',
          size: 14
    ...
    this.chart.options.scales.weight = {
      ...
      position: 'bottom',
      min: 0,
      max: this.#chartScale(Pro.WEIGHT, 0.02).max,
      grid:
      {
        color: ProChart.COLOR.WEIGHT,
        display: false
      },
      ticks:
      {
    ...
    this.chart.options.plugins.legend = {
      labels: {
        filter: (legendItem, chart) => {
          if (legendItem.text.includes('Passive')) return false
          if (legendItem.text.includes('Expansion')) return false
          else return true
        }
      }
    }
    ...
    // chart height
    this.#chartContainer.style.height = ((this.#proList.length * 32) + 103) + 'px'

    // preserve hidden states
    this.chart.legend.legendItems.forEach((legend, index) => {
      this.chart.getDatasetMeta(index).hidden = legend.hidden
    })
